...
public class TrayView implements ActionListener, MouseListener, ItemListener {
	...
	private HashMap<CheckboxMenuItem, String> signatureCheckboxGroup;
	...
	Logger log = MailSignerClient.getLog();
	...
	public void prepareLayout() {
		...
		signatureCheckboxGroup = new HashMap<CheckboxMenuItem, String>();
...
//					continue;
//				}
//				//TODO: Working the garbage collector here!
...
//	}
	...
	public void mousePressed(MouseEvent e) {
		if(e.getSource() == ti) {
			try {
				List<String> userSignatures = trayControl.getUserSignatures();
				this.userSignatures = userSignatures;
				signatureMenu.removeAll();
				signatureCheckboxGroup.clear();
				for (String string : userSignatures) {
					CheckboxMenuItem signatureCheckBox = new CheckboxMenuItem(string);
					signatureMenu.add(signatureCheckBox);
					signatureCheckBox.addItemListener(this);
					signatureCheckboxGroup.put(signatureCheckBox, string);
				}
		...
		if(formatCheckboxGroup.contains(e.getSource())) {
			for (CheckboxMenuItem item : formatCheckboxGroup) {
				if(item != e.getSource()) {
					...
					int i = formatCheckboxGroup.indexOf(item);
					selectedFormat = formatCheckboxGroup.indexOf(item);
				}
			}
		} else {
			Set<Entry<CheckboxMenuItem, String>> entrySet = signatureCheckboxGroup.entrySet();
			for (Entry<CheckboxMenuItem, String> entry : entrySet) {
